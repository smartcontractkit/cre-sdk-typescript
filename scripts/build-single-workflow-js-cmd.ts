...
export const main = async (...args: string[]) => {
	...
	if (args.length === 1) {
		...
		pattern = `src/workflows/${workflowName}/test.ts`
		outputPath = `dist/workflows/${workflowName}`
		...
		pattern = `src/workflows/${directory}/${workflowName}/test.ts`
		outputPath = `dist/workflows/${directory}/${workflowName}`
	} else {
