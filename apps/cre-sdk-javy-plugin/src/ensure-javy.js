...
function computeAsset({ version }) {
  ...
  throw new Error(
    `Unsupported platform for Javy ${version}: ${platform}/${arch}`
  );
}
...
function verifySha256(buffer, expectedHex) {
...
export async function ensureJavy({ version = "v5.0.4" } = {}) {
  ...
  try {
    const [shaText, gzBuf] = await Promise.all([
      downloadText(shaUrl),
