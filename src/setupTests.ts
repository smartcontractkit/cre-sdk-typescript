...
globalThis.awaitCapabilities = mock((_awaitRequest: string, _maxResponseLen: number) => btoa('mock_await_cabilities_response'));
globalThis.getSecrets = mock((_request: string, _maxResponseLen: number) => 1);
globalThis.awaitSecrets = mock((_awaitRequest: string, _maxResponseLen: number) => btoa('mock_await_secrets_response'));
globalThis.log = mock((message: string) => console.log(message))
globalThis.sendResponse = mock((_response: string) => 0);
