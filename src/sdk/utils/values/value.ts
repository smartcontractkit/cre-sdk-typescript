...
export const val = {
	string: (s: string): Value => create(ValueSchema, { value: { case: 'stringValue', value: s } }),
	bool: (b: boolean): Value => create(ValueSchema, { value: { case: 'boolValue', value: b } }),
	...
	list: (items: Array<unknown | Value>): Value => {
		const fields = items.map((i) =>
			typeof i === 'object' && i !== null && (i as Value).value ? (i as Value) : wrapInternal(i),
		)
		return create(ValueSchema, {
	...
	mapValue: (obj: Record<string, unknown | Value>): Value => {
		...
		for (const [k, v] of Object.entries(obj)) {
			fields[k] =
				typeof v === 'object' && v !== null && (v as Value).value ? (v as Value) : wrapInternal(v)
		}
		return create(ValueSchema, {
