...
const doLog0 = (_config: Config, _runtime: Runtime, output: Outputs) => {
	cre.sendResponseValue(cre.utils.val.string(`called 0 with ${output.coolOutput}`))
}

const doLog1 = (_config: Config, _runtime: Runtime, output: TriggerEvent) => {
	cre.sendResponseValue(cre.utils.val.string(`called 1 with ${output.coolOutput}`))
}

const doLog2 = (_config: Config, _runtime: Runtime, output: Outputs) => {
	cre.sendResponseValue(cre.utils.val.string(`called 2 with ${output.coolOutput}`))
}
