...
/**
 * Resolves extension paths to { crateName, path, type }.
 *
 * Auto-detects based on what's provided:
 *   - .rlib file  → pre-compiled rlib (linked via --extern RUSTFLAGS)
 *   - directory    → source crate (compiled by Cargo, name read from Cargo.toml)
 *   - Cargo.toml  → source crate
 */
export function resolveExtensions(extensionPaths: string[]): ExtensionInfo[] {
	return extensionPaths.map((p) => {
		const resolved = resolve(p)

		if (resolved.endsWith('.rlib')) {
			const file = basename(resolved, '.rlib')
			return { crateName: file, path: resolved, type: 'rlib' as const }
		}
		...
		return { crateName, path: dir, type: 'source' as const }
	})
}
 ...
 * @param extensions - Extension crates (source or rlib, as returned by resolveExtensions)
 */
export function generateHostCrate(
	...
	const sourceDeps = extensions.filter((e) => e.type === 'source')
