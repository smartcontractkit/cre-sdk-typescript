    ...
    fn from_js(ctx: &Ctx<'js>, v: Value<'js>) -> Result<Self, Error> {
        ...
        if let Ok(s) = String::from_js(ctx, v) {
            let decoded = base64::engine::general_purpose::STANDARD
                .decode(s.as_bytes())
                .map_err(|_| Exception::throw_type(
                    ctx,
...
fn modify_runtime(runtime: Runtime) -> Runtime {
    runtime.context().with(|ctx| {
        ...
        ctx.globals()
            .set(
                ...
                Func::from(|ctx: Ctx<'_>, req: ArgBytes, max_len: i32| {
                    ...
                    if n < 0 {
                        return Err(Exception::throw_message(&ctx, "get_secrets failed"));
                    }
        ...
        ctx.globals()
            .set(
                ...
                Func::from(|ctx: Ctx<'_>, req: ArgBytes, max_len: i32| {
                    ...
                    if n < 0 {
                        return Err(Exception::throw_message(&ctx, "await_secrets failed"));
                    }
        ...
        ctx.globals()
            .set(
                "getWasiArgs",
                Func::from(|ctx: Ctx<'_>| -> Result<String, Error> {
                    let args: Vec<String> = env::args().collect();
                    let args_json = serde_json::to_string(&args)
                        .map_err(|_| Exception::throw_message(&ctx, "Failed to serialize args to JSON"))?;
                    Ok(args_json)
                }),
            )
            .unwrap();
        ...
        ctx.globals()
            .set(
                "now",
                Func::from(|ctx: Ctx<'_>| -> Result<f64, Error> {
