...
// Reference to the user-defined initialize_runtime export.
// javy v5.0.4 may call compile_src without calling initialize_runtime first,
// so we lazily trigger it.
extern "C" {
    #[link_name = "initialize_runtime"]
    fn user_initialize_runtime();
}

unsafe fn ensure_runtime_initialized() {
    if RUNTIME.get().is_none() {
        user_initialize_runtime();
    }
}
...
pub unsafe extern "C" fn compile_src(js_src_ptr: *const u8, js_src_len: usize) -> *const u32 {
    ...
    unsafe { ensure_runtime_initialized(); }
