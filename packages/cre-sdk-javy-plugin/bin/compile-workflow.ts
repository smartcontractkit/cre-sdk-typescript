...
async function main() {
	...
	if (rest.length < 2) {
		console.error(
			'Usage: compile-workflow.ts [--plugin <path>] [--cre-exports <path>]... <input.js> <output.wasm>',
		)
		console.error('  --plugin: use pre-built .plugin.wasm (mutually exclusive with --cre-exports)')
		console.error(
			'  --cre-exports: build from source with extension crates (mutually exclusive with --plugin)',
		)
	...
	if (pluginArg !== null) {
		// Mode (c): use user-provided pre-built plugin
		...
		// Mode (b): build from source with extensions
		...
		// Mode (a): use default pre-built plugin from dist/
