...
var init_base = __esm(() => {
	...
	BaseError = class BaseError2 extends Error {
		constructor(shortMessage, args = {}) {
			const details = (() => {
				if (args.cause instanceof BaseError2) return args.cause.details
				if (args.cause?.message) return args.cause.message
				return args.details
			})()
			const docsPath = (() => {
				if (args.cause instanceof BaseError2) return args.cause.docsPath || args.docsPath
...
function pad(hexOrBytes, { dir, size: size2 = 32 } = {}) {
	if (typeof hexOrBytes === 'string') return padHex(hexOrBytes, { dir, size: size2 })
...
var init_hmac = __esm(() => {
	...
	HMAC = class HMAC2 extends Hash {
		constructor(hash, _key) {
			...
			for (let i = 0; i < pad2.length; i++) pad2[i] ^= 54
			this.iHash.update(pad2)
			this.oHash = hash.create()
			for (let i = 0; i < pad2.length; i++) pad2[i] ^= 54 ^ 92
...
function FpPow(Fp, num, power) {
	...
	if (power === _0n3) return Fp.ONE
	if (power === _1n3) return num
	let p = Fp.ONE
	let d = num
	while (power > _0n3) {
		if (power & _1n3) p = Fp.mul(p, d)
		d = Fp.sqr(d)
		power >>= _1n3
	}
	return p
...
function Field(ORDER, bitLenOrOpts, isLE2 = false, opts = {}) {
	...
	if (typeof bitLenOrOpts === 'object' && bitLenOrOpts != null) {
		if (opts.sqrt || isLE2) throw new Error('cannot specify opts in two arguments')
		const _opts = bitLenOrOpts
		if (_opts.BITS) _nbitLength = _opts.BITS
		if (_opts.sqrt) _sqrt = _opts.sqrt
		if (typeof _opts.isLE === 'boolean') isLE2 = _opts.isLE
...
function assert0(n) {
	if (n !== _0n4) throw new Error('invalid wNAF')
}
...
function mulEndoUnsafe(Point, point, k1, k2) {
	...
	while (k1 > _0n4 || k2 > _0n4) {
		if (k1 & _1n4) p1 = p1.add(acc)
		if (k2 & _1n4) p2 = p2.add(acc)
		acc = acc.double()
		k1 >>= _1n4
		k2 >>= _1n4
	}
...
function weierstrassN(params, extraOpts = {}) {
	...
	class Point {
		...
		multiplyUnsafe(sc) {
			...
			if (sc === _0n5 || p.is0()) return Point.ZERO
			if (sc === _1n5) return p
...
function SWUFpSqrtRatio(Fp, Z) {
	const q = Fp.ORDER
	let l = _0n5
	for (let o = q - _1n5; o % _2n3 === _0n5; o /= _2n3) l += _1n5
	const c1 = l
...
function ecdh(Point, ecdhOpts = {}) {
	...
	function isProbPub(item) {
		...
		if (Fn.allowedLengths || secretKey === publicKey) return
...
function schnorrVerify(signature, message, publicKey) {
	...
	try {
		...
		if (!inRange(r, _1n6, secp256k1_CURVE.p)) return false
		const s = num(sig.subarray(32, 64))
		if (!inRange(s, _1n6, secp256k1_CURVE.n)) return false
...
function isMessage(arg, schema) {
	const isMessage2 =
		arg !== null && typeof arg == 'object' && '$typeName' in arg && typeof arg.$typeName == 'string'
	if (!isMessage2) {
		return false
	}
	if (schema === undefined) {
	...
	if (
		...
		if (field !== undefined) {
			...
			return (
				...
				a.scalar === b.scalar &&
				((_a = a.message) === null || _a === undefined ? undefined : _a.typeName) ===
					((_b = b.message) === null || _b === undefined ? undefined : _b.typeName) &&
				((_c = a.enum) === null || _c === undefined ? undefined : _c.typeName) ===
...
var errorNames = ['FieldValueInvalidError', 'FieldListRangeError', 'ForeignFieldError']
...
var reservedObjectProperties = new Set(['constructor', 'toString', 'toJSON', 'valueOf'])
...
function createBaseRegistry() {
	...
	return {
		...
		getExtensionFor(extendee, no) {
			var _a
			return (_a = extendees.get(extendee.typeName)) === null || _a === undefined
				? undefined
				: _a.get(no)
		},
		getService(typeName) {
...
function addFile(proto, reg) {
	...
	const file = {
		...
		proto,
		deprecated:
			(_b = (_a = proto.options) === null || _a === undefined ? undefined : _a.deprecated) !==
				null && _b !== undefined
				? _b
				: false,
	...
	const mapEntries = {
		...
		add(desc) {
			var _a2
			assert(
				((_a2 = desc.proto.options) === null || _a2 === undefined ? undefined : _a2.mapEntry) ===
					true,
			)
...
function addEnum(proto, file, parent, reg) {
	...
	for (const p of proto.value) {
		...
		desc.values.push(
			(desc.value[p.number] = {
				...
				proto: p,
				deprecated:
					(_d = (_c = p.options) === null || _c === undefined ? undefined : _c.deprecated) !==
						null && _d !== undefined
						? _d
						: false,
				parent: desc,
	...
	;((_e = parent === null || parent === undefined ? undefined : parent.nestedEnums) !== null &&
	_e !== undefined
		? _e
		: file.enums
	).push(desc)
...
function addMessage(proto, file, parent, reg, mapEntries) {
	...
	if (((_c = proto.options) === null || _c === undefined ? undefined : _c.mapEntry) === true) {
		...
		;((_d = parent === null || parent === undefined ? undefined : parent.nestedMessages) !== null &&
		_d !== undefined
			? _d
			: file.messages
		).push(desc)
...
function newMethod(proto, parent, reg) {
	...
	return {
		...
		name,
		localName: safeObjectProperty(
			name.length ? safeObjectProperty(name[0].toLowerCase() + name.substring(1)) : name,
		),
		methodKind,
		input,
		output,
		idempotency:
			(_d = (_c = proto.options) === null || _c === undefined ? undefined : _c.idempotencyLevel) !==
				null && _d !== undefined
				? _d
				: IDEMPOTENCY_UNKNOWN,
		toString() {
		...
		extensionRange:
			(_h =
				(_g = init.extensionRange) === null || _g === undefined
					? undefined
					: _g.map((e) =>
							Object.assign({ $typeName: 'google.protobuf.DescriptorProto.ExtensionRange' }, e),
						)) !== null && _h !== undefined
				? _h
				: [],
		oneofDecl: [],
...
function bootFieldDescriptorProto(init) {
	...
	return Object.assign(
		proto,
		Object.assign(Object.assign({ $typeName: 'google.protobuf.FieldDescriptorProto' }, init), {
			...
			editionDefaults:
				(_c =
					(_b = init.editionDefaults) === null || _b === undefined
						? undefined
						: _b.map((e) =>
								Object.assign({ $typeName: 'google.protobuf.FieldOptions.EditionDefault' }, e),
							)) !== null && _c !== undefined
					? _c
					: [],
			uninterpretedOption: [],
...
function bootEnumDescriptorProto(init) {
	...
	return Object.assign(proto, {
		...
		value: init.value.map((e) =>
			Object.assign({ $typeName: 'google.protobuf.EnumValueDescriptorProto' }, e),
		),
	})
}
	...
	messageType: [
		...
		},
		{
			...
			field: [
				...
				{ name: 'edition', number: 14, type: 14, label: 1, typeName: '.google.protobuf.Edition' },
			],
		},
		{
		...
		},
		{
			...
			field: [
				...
				{ name: 'client_streaming', number: 5, type: 8, label: 1, defaultValue: 'false' },
				{ name: 'server_streaming', number: 6, type: 8, label: 1, defaultValue: 'false' },
			],
		},
		{
			name: 'FileOptions',
			field: [
				...
				{ name: 'java_multiple_files', number: 10, type: 8, label: 1, defaultValue: 'false' },
				{
				...
				{ name: 'java_string_check_utf8', number: 27, type: 8, label: 1, defaultValue: 'false' },
				{
					name: 'optimize_for',
					number: 9,
					type: 14,
				...
				{ name: 'cc_generic_services', number: 16, type: 8, label: 1, defaultValue: 'false' },
				{ name: 'java_generic_services', number: 17, type: 8, label: 1, defaultValue: 'false' },
				{ name: 'py_generic_services', number: 18, type: 8, label: 1, defaultValue: 'false' },
				{ name: 'deprecated', number: 23, type: 8, label: 1, defaultValue: 'false' },
				{ name: 'cc_enable_arenas', number: 31, type: 8, label: 1, defaultValue: 'true' },
		...
		},
		{
			name: 'MessageOptions',
			field: [
				{ name: 'message_set_wire_format', number: 1, type: 8, label: 1, defaultValue: 'false' },
				{
		...
		},
		{
			...
			field: [
				...
				{ name: 'unverified_lazy', number: 15, type: 8, label: 1, defaultValue: 'false' },
				{ name: 'deprecated', number: 3, type: 8, label: 1, defaultValue: 'false' },
			...
			nestedType: [
				{
					name: 'Location',
					field: [
						{ name: 'path', number: 1, type: 5, label: 3, options: { packed: true } },
						{ name: 'span', number: 2, type: 5, label: 3, options: { packed: true } },
...
;(function (FieldOptions_OptionRetention2) {
	FieldOptions_OptionRetention2[(FieldOptions_OptionRetention2['RETENTION_UNKNOWN'] = 0)] =
		'RETENTION_UNKNOWN'
	FieldOptions_OptionRetention2[(FieldOptions_OptionRetention2['RETENTION_RUNTIME'] = 1)] =
		'RETENTION_RUNTIME'
	FieldOptions_OptionRetention2[(FieldOptions_OptionRetention2['RETENTION_SOURCE'] = 2)] =
		'RETENTION_SOURCE'
})(FieldOptions_OptionRetention || (FieldOptions_OptionRetention = {}))
var FieldOptions_OptionTargetType
;(function (FieldOptions_OptionTargetType2) {
	FieldOptions_OptionTargetType2[(FieldOptions_OptionTargetType2['TARGET_TYPE_UNKNOWN'] = 0)] =
		'TARGET_TYPE_UNKNOWN'
	FieldOptions_OptionTargetType2[(FieldOptions_OptionTargetType2['TARGET_TYPE_FILE'] = 1)] =
		'TARGET_TYPE_FILE'
	FieldOptions_OptionTargetType2[
		(FieldOptions_OptionTargetType2['TARGET_TYPE_EXTENSION_RANGE'] = 2)
	] = 'TARGET_TYPE_EXTENSION_RANGE'
	FieldOptions_OptionTargetType2[(FieldOptions_OptionTargetType2['TARGET_TYPE_MESSAGE'] = 3)] =
		'TARGET_TYPE_MESSAGE'
	FieldOptions_OptionTargetType2[(FieldOptions_OptionTargetType2['TARGET_TYPE_FIELD'] = 4)] =
		'TARGET_TYPE_FIELD'
	FieldOptions_OptionTargetType2[(FieldOptions_OptionTargetType2['TARGET_TYPE_ONEOF'] = 5)] =
		'TARGET_TYPE_ONEOF'
	FieldOptions_OptionTargetType2[(FieldOptions_OptionTargetType2['TARGET_TYPE_ENUM'] = 6)] =
		'TARGET_TYPE_ENUM'
	FieldOptions_OptionTargetType2[(FieldOptions_OptionTargetType2['TARGET_TYPE_ENUM_ENTRY'] = 7)] =
		'TARGET_TYPE_ENUM_ENTRY'
	FieldOptions_OptionTargetType2[(FieldOptions_OptionTargetType2['TARGET_TYPE_SERVICE'] = 8)] =
		'TARGET_TYPE_SERVICE'
	FieldOptions_OptionTargetType2[(FieldOptions_OptionTargetType2['TARGET_TYPE_METHOD'] = 9)] =
		'TARGET_TYPE_METHOD'
})(FieldOptions_OptionTargetType || (FieldOptions_OptionTargetType = {}))
var MethodOptions_IdempotencyLevel
;(function (MethodOptions_IdempotencyLevel2) {
	MethodOptions_IdempotencyLevel2[(MethodOptions_IdempotencyLevel2['IDEMPOTENCY_UNKNOWN'] = 0)] =
		'IDEMPOTENCY_UNKNOWN'
	MethodOptions_IdempotencyLevel2[(MethodOptions_IdempotencyLevel2['NO_SIDE_EFFECTS'] = 1)] =
		'NO_SIDE_EFFECTS'
	MethodOptions_IdempotencyLevel2[(MethodOptions_IdempotencyLevel2['IDEMPOTENT'] = 2)] =
		'IDEMPOTENT'
...
;(function (FeatureSet_VisibilityFeature_DefaultSymbolVisibility2) {
	FeatureSet_VisibilityFeature_DefaultSymbolVisibility2[
		(FeatureSet_VisibilityFeature_DefaultSymbolVisibility2['DEFAULT_SYMBOL_VISIBILITY_UNKNOWN'] = 0)
	] = 'DEFAULT_SYMBOL_VISIBILITY_UNKNOWN'
	FeatureSet_VisibilityFeature_DefaultSymbolVisibility2[
		(FeatureSet_VisibilityFeature_DefaultSymbolVisibility2['EXPORT_ALL'] = 1)
	] = 'EXPORT_ALL'
	FeatureSet_VisibilityFeature_DefaultSymbolVisibility2[
		(FeatureSet_VisibilityFeature_DefaultSymbolVisibility2['EXPORT_TOP_LEVEL'] = 2)
	] = 'EXPORT_TOP_LEVEL'
	FeatureSet_VisibilityFeature_DefaultSymbolVisibility2[
...
;(function (FeatureSet_FieldPresence2) {
	FeatureSet_FieldPresence2[(FeatureSet_FieldPresence2['FIELD_PRESENCE_UNKNOWN'] = 0)] =
...
;(function (FeatureSet_RepeatedFieldEncoding2) {
	...
	FeatureSet_RepeatedFieldEncoding2[(FeatureSet_RepeatedFieldEncoding2['EXPANDED'] = 2)] =
		'EXPANDED'
})(FeatureSet_RepeatedFieldEncoding || (FeatureSet_RepeatedFieldEncoding = {}))
var FeatureSet_Utf8Validation
;(function (FeatureSet_Utf8Validation2) {
	FeatureSet_Utf8Validation2[(FeatureSet_Utf8Validation2['UTF8_VALIDATION_UNKNOWN'] = 0)] =
...
;(function (FeatureSet_MessageEncoding2) {
	FeatureSet_MessageEncoding2[(FeatureSet_MessageEncoding2['MESSAGE_ENCODING_UNKNOWN'] = 0)] =
		'MESSAGE_ENCODING_UNKNOWN'
	FeatureSet_MessageEncoding2[(FeatureSet_MessageEncoding2['LENGTH_PREFIXED'] = 1)] =
		'LENGTH_PREFIXED'
...
;(function (FeatureSet_EnforceNamingStyle2) {
	...
	FeatureSet_EnforceNamingStyle2[(FeatureSet_EnforceNamingStyle2['STYLE_LEGACY'] = 2)] =
		'STYLE_LEGACY'
})(FeatureSet_EnforceNamingStyle || (FeatureSet_EnforceNamingStyle = {}))
var GeneratedCodeInfo_Annotation_Semantic
;(function (GeneratedCodeInfo_Annotation_Semantic2) {
	GeneratedCodeInfo_Annotation_Semantic2[(GeneratedCodeInfo_Annotation_Semantic2['NONE'] = 0)] =
		'NONE'
	GeneratedCodeInfo_Annotation_Semantic2[(GeneratedCodeInfo_Annotation_Semantic2['SET'] = 1)] =
		'SET'
	GeneratedCodeInfo_Annotation_Semantic2[(GeneratedCodeInfo_Annotation_Semantic2['ALIAS'] = 2)] =
		'ALIAS'
...
function fileDesc(b64, imports) {
	...
	root.dependency =
		(_a =
			imports === null || imports === undefined ? undefined : imports.map((f) => f.proto.name)) !==
			null && _a !== undefined
			? _a
			: []
...
function writeFields(writer, opts, msg) {
	...
	if (opts.writeUnknownFields) {
		for (const { no, wireType, data } of (_a = msg.getUnknown()) !== null && _a !== undefined
			? _a
			: []) {
...
function writeMessageField(writer, opts, field, message) {
	if (field.delimitedEncoding) {
		writeFields(writer.tag(field.number, WireType.StartGroup), opts, message).tag(
			field.number,
			WireType.EndGroup,
		)
	} else {
...
function setExtension(message, extension, value) {
	...
	const ufs = ((_a = message.$unknown) !== null && _a !== undefined ? _a : []).filter(
...
function anyFromJson(any, json, opts) {
	...
	const desc =
		(_a = opts.registry) === null || _a === undefined ? undefined : _a.getMessage(typeName)
	if (!desc) {
		throw new Error(
...
async function recoverPublicKey({ hash, signature }) {
	...
	const { secp256k1: secp256k12 } = await Promise.resolve().then(
		() => (init_secp256k1(), exports_secp256k1),
	)
	const signature_ = (() => {
		...
		return secp256k12.Signature.fromCompact(signatureHex.substring(2, 130)).addRecoveryBit(
			recoveryBit,
		)
	})()
...
var CALL_CONTRACT_REQUEST_TYPE_URL =
...
function encodeGetNodesByP2PIdsCalldata(p2pIds) {
	const chunks = [GET_NODES_BY_P2P_IDS_SELECTOR, padUint256(32), padUint256(p2pIds.length)]
...
var file_capabilities_networking_confidentialhttp_v1alpha_client = /* @__PURE__ */ fileDesc(
...
var HTTPResponseSchema = /* @__PURE__ */ messageDesc(
	file_capabilities_networking_confidentialhttp_v1alpha_client,
	3,
)
var ConfidentialHTTPRequestSchema = /* @__PURE__ */ messageDesc(
	file_capabilities_networking_confidentialhttp_v1alpha_client,
	4,
)
...
var file_capabilities_scheduler_cron_v1_trigger = /* @__PURE__ */ fileDesc(
...
function assertSize2(size2) {
	if (typeof size2 !== 'number') throw TypeError('"size" argument must be of type number')
	else if (size2 < 0) throw RangeError('The value "' + size2 + '" is invalid for option "size"')
}
function alloc(size2, fill, encoding) {
	if ((assertSize2(size2), size2 <= 0)) return createBuffer(size2)
	if (fill !== undefined)
		return typeof encoding === 'string'
			? createBuffer(size2).fill(fill, encoding)
			: createBuffer(size2).fill(fill)
...
function byteLength(string, encoding) {
	...
	for (;;)
		switch (encoding) {
			...
			default:
				if (loweredCase) return mustMatch ? -1 : utf8ToBytes2(string).length
			...
			switch (bytesPerSequence) {
				...
				case 2:
					if (((secondByte = buf[i2 + 1]), (secondByte & 192) === 128)) {
						if (
							((tempCodePoint = ((firstByte & 31) << 6) | (secondByte & 63)), tempCodePoint > 127)
						)
...
function base64clean(str) {
	if (
		((str = str.split('=')[0]), (str = str.trim().replace(INVALID_BASE64_RE, '')), str.length < 2)
	)
		return ''
...
Url.prototype.parse = function (url, parseQueryString, slashesDenoteHost) {
	if (!util_isString(url)) throw TypeError("Parameter 'url' must be a string, not " + typeof url)
	...
	if (!hostlessProtocol[proto] && (slashes || (proto && !slashedProtocol[proto]))) {
		...
		if (!ipv6Hostname) this.hostname = new URL2(`https://${this.hostname}`).hostname
	...
	if (hash !== -1) (this.hash = rest.substr(hash)), (rest = rest.slice(0, hash))
	var qm = rest.indexOf('?')
	if (qm !== -1) {
...
Url.prototype.format = function () {
	var auth = this.auth || ''
	if (auth) (auth = encodeURIComponent(auth)), (auth = auth.replace(/%3A/i, ':')), (auth += '@')
...
Url.prototype.resolveObject = function (relative) {
	...
	if (relative.protocol && relative.protocol !== result.protocol) {
		...
		return (
			(result.slashes = result.slashes || relative.slashes), (result.href = result.format()), result
		)
	}
	...
	if (!srcPath.length) {
		if (((result.pathname = null), result.search)) result.path = '/' + result.search
...
var testnetByName = new Map([
	...
	['ethereum-testnet-sepolia-polygon-zkevm-1', ethereum_testnet_sepolia_polygon_zkevm_1_default],
	...
	['ethereum-testnet-sepolia-arbitrum-1-l3x-1', ethereum_testnet_sepolia_arbitrum_1_l3x_1_default],
	[
...
function httpRequest(opts) {
	...
	if (opts.multiHeaders || opts.headers) {
		out.multiHeaders = {}
		if (opts.multiHeaders) {
			for (const [name, value2] of Object.entries(opts.multiHeaders)) {
				out.multiHeaders[name] = { values: [...(value2.values ?? [])] }
			}
		}
		if (opts.headers) {
			for (const [name, value2] of Object.entries(opts.headers)) {
				out.multiHeaders[name] = {
...
var globalHostBindingsSchema = exports_external.object({
	...
	sendResponse: exports_external
		.function()
		.args(
			exports_external.union([exports_external.instanceof(Uint8Array), exports_external.custom()]),
		)
...
var onCronTrigger = (runtime2) => {
	const client = new ClientCapability()
	const separateRequestConfig = {
		request: httpRequest({
			url: runtime2.config.url,
			method: 'POST',
	...
	client.sendRequest(runtime2, separateRequestConfig).result()
	client
		.sendRequest(runtime2, {
			request: httpRequest({
				url: runtime2.config.url,
				method: 'POST',
				body: { hello: 'world' },
