...
describe("TestRuntime / helper layer", () => {
	...
	test("default Report handler: defaultReport metadata + payload + sigs", () => {
		...
		const result = rt
			.report({ encodedPayload: Buffer.from(payload).toString("base64") })
			.result();
		const unwrapped = result.x_generatedCodeOnly_unwrap();
		expect(unwrapped.rawReport.length).toBe(
			REPORT_METADATA_HEADER_LENGTH + payload.length,
		);
		...
		expect(unwrapped.rawReport.slice(0, REPORT_METADATA_HEADER_LENGTH)).toEqual(
			expectedMetadata,
		);
		expect(unwrapped.rawReport.slice(REPORT_METADATA_HEADER_LENGTH)).toEqual(
			payload,
		);
		expect(unwrapped.sigs).toHaveLength(2);
		expect(new TextDecoder().decode(unwrapped.sigs[0].signature)).toBe(
			"default_signature_1",
		);
		expect(new TextDecoder().decode(unwrapped.sigs[1].signature)).toBe(
			"default_signature_2",
		);
	});
	...
	test("helper getSecrets: batched call returns mixed secret/error responses", () => {
		const secrets = new Map<string, Map<string, string>>();
		secrets.set("ns1", new Map([["id1", "val1"]]));
		const rt = newTestRuntime(secrets);
		...
		expect(responses[0].response.case).toBe("secret");
		expect(responses[1].response.case).toBe("error");
	});

	test("helper getSecrets: secret not found returns error response", () => {
		const rt = newTestRuntime();
		expect(() =>
			rt.getSecret({ id: "missing", namespace: "ns" }).result(),
	...
	test("newTestRuntime rejects unsafe maxResponseSize values", () => {
		expect(() =>
			newTestRuntime(null, {
				maxResponseSize: Number.MAX_SAFE_INTEGER + 1,
			}),
		).toThrow(
	...
	test("newTestRuntime with null/undefined secrets uses empty map", () => {
		...
		expect(() => rt.getSecret({ id: "x", namespace: "y" }).result()).toThrow(
			SecretsError,
		);
