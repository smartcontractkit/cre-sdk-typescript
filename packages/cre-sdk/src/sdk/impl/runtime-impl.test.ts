...
const originalNodeActionPerformAction =
	NodeActionCapability.prototype.performAction;
...
afterEach(() => {
	...
	NodeActionCapability.prototype.performAction =
		originalNodeActionPerformAction;
});
...
describe("test runtime", () => {
	describe("test call capability", () => {
				...
				call: mock((request: CapabilityRequest) => {
					switch (request.callbackId) {
						case 1:
							...
							expect(anyUnpack(request.payload as Any, InputsSchema)).toEqual(
								input1,
							);
							return true;
						case 2:
							...
							expect(anyUnpack(request.payload as Any, InputSchema)).toEqual(
								input2,
							);
							return true;
						default:
							throw new Error(
								`Unexpected call with callbackId: ${request.callbackId}`,
							);
					}
				}),
				await: mock((request: AwaitCapabilitiesRequest) => {
					...
					switch (id) {
						case 1:
							...
							payload = anyPack(
								OutputsSchema,
								create(OutputsSchema, { adaptedThing: anyResult1 }),
							);
							break;
						case 2:
							...
							payload = anyPack(
								OutputSchema,
								create(OutputSchema, { welcome: anyResult2 }),
							);
							break;
						default:
		...
		test('await returns unparsable payload throws CapabilityRuntimeError', () => {
			...
			const validAny = anyPack(
				OutputsSchema,
				create(OutputsSchema, { adaptedThing: "x" }),
			);
...
describe("test getSecret", () => {
	...
	test("normalizes missing secret namespace to default for JSON and protobuf requests", () => {
		...
		runtime
			.getSecret(create(SecretRequestSchema, { id: "proto-secret" }))
			.result();
...
describe("test run in node mode", () => {
	test("successful consensus", () => {
		...
		(ConsensusCapability.prototype as any).simple = mock(
			(
				...
				expect(
					(inputs as { $typeName?: string }).$typeName,
				).not.toBeUndefined();
				...
				return {
					result: () =>
						Value.from(
							create(NodeOutputsSchema, { outputThing: anyMedian }),
						).proto(),
				};
			},
		);
		...
		const performActionMock = function (
			this: NodeActionCapability,
			...args: unknown[]
		): unknown {
			...
			const [_, __] = args as [
				NodeRuntime<unknown>,
				NodeInputs | NodeInputsJson,
			];
			expect(modes).toEqual([Mode.DON, Mode.NODE]);
			return {
				result: () =>
					create(NodeOutputsSchema, { outputThing: anyObservation }),
			};
		};
		...
		(NodeActionCapability.prototype as any).performAction =
			mock(performActionMock);
	...
	test("primitive consensus with unused default returns observation value", () => {
		...
		(ConsensusCapability.prototype as any).simple = mock(
			(
				...
				expect(inputsProto.observation.case).toEqual("value");
				expect(
					Value.wrap(inputsProto.observation.value as ProtoValue).unwrap(),
				).toEqual(observationValue);
				expect(inputsProto.default).toBeDefined();
				expect(Value.wrap(inputsProto.default as ProtoValue).unwrap()).toEqual(
					defaultValue,
				);
				return {
	...
	test("node runtime in don mode fails", () => {
		...
		(ConsensusCapability.prototype as any).simple = mock(
			(
				_: Runtime<unknown>,
				__: SimpleConsensusInputs | SimpleConsensusInputsJson,
			) => {
				return { result: () => Value.from(0).proto() };
			},
		);
	...
	test("don runtime in node mode fails", () => {
		...
		(ConsensusCapability.prototype as any).simple = mock(
			(
				...
				const inputsProto = inputs as SimpleConsensusInputs;
				expect(inputsProto.observation.case).toEqual("error");
				expect(inputsProto.observation.value).toEqual(
					new DonModeError().message,
				);
				return {
					result: () => {
	...
	test("multiple runInNodeMode calls have unique callback IDs", () => {
					...
					responses: {
						[id]: create(CapabilityResponseSchema, {
							response: {
								...
								value: anyPack(
									NodeOutputsSchema,
									create(NodeOutputsSchema, { outputThing: 42 }),
								),
							},
						}),
					},
		...
		(ConsensusCapability.prototype as any).simple = mock(
			(
				...
				return {
					result: () =>
						Value.from(create(NodeOutputsSchema, { outputThing: 42 })).proto(),
				};
			},
		);
		...
		const call2 = runtime.runInNodeMode(
			(nodeRuntime: NodeRuntime<unknown>) => {
				...
				return capability
					.performAction(
						nodeRuntime,
						create(NodeInputsSchema, { inputThing: false }),
					)
					.result();
			},
			ConsensusAggregationByFields<NodeOutputs>({ outputThing: median }),
		...
		(ConsensusCapability.prototype as any).simple = mock(
			(
				...
				if (inputsProto.observation.case === "value") {
					...
					expect(unwrapped.nested.nestedIncluded).toEqual(
						"response_nested_included",
					);
				...
				if (inputsProto.default) {
					const unwrapped = Value.wrap(
						inputsProto.default as ProtoValue,
					).unwrap() as TestStruct;
					expect(unwrapped.includedField).toEqual("default_included");
					expect(unwrapped.ignoredField).toBeUndefined();
					expect(unwrapped.nested.nestedIncluded).toEqual(
						"default_nested_included",
					);
					expect(unwrapped.nested.nestedIgnored).toBeUndefined();
					return {
		...
		const result = runtime
			.runInNodeMode(
				...
				ConsensusAggregationByFields<TestStruct>({
					...
					ignoredField: ignore,
					nested: () =>
						new ConsensusFieldAggregation<NestedStruct, true>(
							nestedAggregation.descriptor,
						),
