...
const anyExecuteRequest = create(ExecuteRequestSchema, {
	request: {
		...
		value: create(TriggerSchema, {
			...
			payload: anyPack(
				OutputsSchema,
				create(OutputsSchema, { coolOutput: "hi" }),
			),
		}),
	},
	maxResponseSize: anyMaxResponseSize,
...
describe("runner", () => {
	describe("run", () => {
		test("gathers subscriptions", async () => {
			...
			await runner.run(async (_: string, secretsProvider: SecretsProvider) => {
				return [
					cre.handler(basicTrigger.trigger({ name: "foo", number: 10 }), () => {
						throw new Error(
							"Must not be called during registration to tiggers",
						);
					}),
				];
			});
			...
			const responseValue = sentResponse!.result
				.value! as TriggerSubscriptionRequest;
			...
			const actualConfig = anyUnpack(
				responseValue.subscriptions[0].payload!,
				ConfigSchema,
			)!;
	...
	test("executes workflow with multiple triggers", async () => {
		...
		await runner.run(async (_: string, secretsProvider: SecretsProvider) => {
			return [
				...
				cre.handler(
					basicTrigger.trigger({ name: "bar", number: 20 }),
					(runtime, trigger) => {
						expect(runtime.config).toBe(anyConfig.toString());
						expect(trigger.coolOutput).toBe("hi");
						return 20;
				...
				cre.handler(
					basicTrigger.trigger({ name: "baz", number: 30 }),
					(runtime, trigger) => {
						expect(runtime.config).toBe(anyConfig.toString());
						expect(trigger.coolOutput).toBe("hi");
						return 30;
	...
	test("get secrets passes max response size", async () => {
		...
		mockHostBindings.awaitSecrets = (data, maxResponseSize) => {
			const dataBytes = Array.isArray(data) ? new Uint8Array(data) : data;
			const awaitSecretsRequest = fromBinary(
				AwaitSecretsRequestSchema,
				dataBytes,
			);
		...
		await (await dr).run(
			async (_: string, secretsProvider: SecretsProvider) => {
				const batched = await secretsProvider
					.getSecrets([{ namespace: "Foo", id: "Bar" }])
					.result();
				...
				const single = await secretsProvider
					.getSecret({ namespace: "Foo", id: "Bar" })
					.result();
...
function getTestRunner(request: ExecuteRequest): Promise<Runner<string>> {
	...
	mockHostBindings.getWasiArgs = mock(() =>
		JSON.stringify(["program", encoded]),
	);
