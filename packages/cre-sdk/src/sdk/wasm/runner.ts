...
export class Runner<TConfig> {
	...
	async handleExecutionPhase<TConfig>(
		...
		return create(ExecutionResultSchema, {
			result: { case: 'error', value: `trigger not found: no workflow handler registered at index ${index} (trigger ID ${triggerMsg.id}). The workflow has ${workflow.length} handler(s) registered. Verify the trigger subscription matches a registered handler` },
		})
	}
	...
	handleSubscribePhase(req: ExecuteRequest, workflow: Workflow<TConfig>): ExecutionResult {
		if (req.request.case !== 'subscribe') {
			return create(ExecutionResultSchema, {
				result: { case: 'error', value: `subscribe request expected but received '${req.request.case}' in handleSubscribePhase. This is an internal SDK error` },
			})
		}
