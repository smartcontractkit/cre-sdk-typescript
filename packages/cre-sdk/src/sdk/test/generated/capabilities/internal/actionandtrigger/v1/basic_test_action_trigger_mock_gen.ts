...
import { type Input, InputSchema, type Output, type OutputJson, OutputSchema } from "@cre/generated/capabilities/internal/actionandtrigger/v1/action_and_trigger_pb"
  ...
  private constructor() {
    ...
    try {
      ...
      case "Action": {
        ...
        if (typeof handler !== "function") throw new Error("Action: no implementation provided; set the mock's action property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as Output) : fromJson(OutputSchema, raw as OutputJson);
