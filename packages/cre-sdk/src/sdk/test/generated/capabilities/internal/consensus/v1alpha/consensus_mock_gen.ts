...
import { type ReportRequest, ReportRequestSchema, type ReportResponse, type ReportResponseJson, ReportResponseSchema, type SimpleConsensusInputs, SimpleConsensusInputsSchema } from "@cre/generated/sdk/v1alpha/sdk_pb"
  ...
  private constructor() {
    ...
    try {
      ...
      case "Simple": {
        ...
        if (typeof handler !== "function") throw new Error("Simple: no implementation provided; set the mock's simple property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as Value) : fromJson(ValueSchema, raw as ValueJson);
      ...
      case "Report": {
        ...
        if (typeof handler !== "function") throw new Error("Report: no implementation provided; set the mock's report property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as ReportResponse) : fromJson(ReportResponseSchema, raw as ReportResponseJson);
