...
import { type NodeInputs, NodeInputsSchema, type NodeOutputs, type NodeOutputsJson, NodeOutputsSchema } from "@cre/generated/capabilities/internal/nodeaction/v1/node_action_pb"
  ...
  private constructor() {
    ...
    try {
      ...
      case "PerformAction": {
        ...
        if (typeof handler !== "function") throw new Error("PerformAction: no implementation provided; set the mock's performAction property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as NodeOutputs) : fromJson(NodeOutputsSchema, raw as NodeOutputsJson);
