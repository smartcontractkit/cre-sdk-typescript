...
import { type ConfidentialHTTPRequest, ConfidentialHTTPRequestSchema, type HTTPResponse, type HTTPResponseJson, HTTPResponseSchema } from "@cre/generated/capabilities/networking/confidentialhttp/v1alpha/client_pb"
  ...
  private constructor() {
    ...
    try {
      ...
      case "SendRequest": {
        ...
        if (typeof handler !== "function") throw new Error("SendRequest: no implementation provided; set the mock's sendRequest property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as HTTPResponse) : fromJson(HTTPResponseSchema, raw as HTTPResponseJson);
