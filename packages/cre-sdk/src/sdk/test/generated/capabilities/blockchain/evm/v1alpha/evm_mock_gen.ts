...
import { registerTestCapability, __getTestMockInstance, __setTestMockInstance } from "@cre/sdk/testutils/test-runtime"
  ...
  private constructor(chainSelector: bigint) {
    ...
    try {
      ...
      case "CallContract": {
        ...
        if (typeof handler !== "function") throw new Error("CallContract: no implementation provided; set the mock's callContract property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as CallContractReply) : fromJson(CallContractReplySchema, raw as CallContractReplyJson);
        return { response: { case: "payload", value: anyPack(CallContractReplySchema, output) } };
      }
      case "FilterLogs": {
        const input = anyUnpack(req.payload, FilterLogsRequestSchema) as FilterLogsRequest;
        const handler = self.filterLogs;
        if (typeof handler !== "function") throw new Error("FilterLogs: no implementation provided; set the mock's filterLogs property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as FilterLogsReply) : fromJson(FilterLogsReplySchema, raw as FilterLogsReplyJson);
      ...
      case "BalanceAt": {
        ...
        if (typeof handler !== "function") throw new Error("BalanceAt: no implementation provided; set the mock's balanceAt property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as BalanceAtReply) : fromJson(BalanceAtReplySchema, raw as BalanceAtReplyJson);
      ...
      case "EstimateGas": {
        ...
        if (typeof handler !== "function") throw new Error("EstimateGas: no implementation provided; set the mock's estimateGas property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as EstimateGasReply) : fromJson(EstimateGasReplySchema, raw as EstimateGasReplyJson);
        return { response: { case: "payload", value: anyPack(EstimateGasReplySchema, output) } };
      }
      case "GetTransactionByHash": {
        const input = anyUnpack(req.payload, GetTransactionByHashRequestSchema) as GetTransactionByHashRequest;
        const handler = self.getTransactionByHash;
        if (typeof handler !== "function") throw new Error("GetTransactionByHash: no implementation provided; set the mock's getTransactionByHash property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as GetTransactionByHashReply) : fromJson(GetTransactionByHashReplySchema, raw as GetTransactionByHashReplyJson);
        return { response: { case: "payload", value: anyPack(GetTransactionByHashReplySchema, output) } };
      }
      case "GetTransactionReceipt": {
        const input = anyUnpack(req.payload, GetTransactionReceiptRequestSchema) as GetTransactionReceiptRequest;
        const handler = self.getTransactionReceipt;
        if (typeof handler !== "function") throw new Error("GetTransactionReceipt: no implementation provided; set the mock's getTransactionReceipt property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as GetTransactionReceiptReply) : fromJson(GetTransactionReceiptReplySchema, raw as GetTransactionReceiptReplyJson);
        return { response: { case: "payload", value: anyPack(GetTransactionReceiptReplySchema, output) } };
      }
      case "HeaderByNumber": {
        const input = anyUnpack(req.payload, HeaderByNumberRequestSchema) as HeaderByNumberRequest;
        const handler = self.headerByNumber;
        if (typeof handler !== "function") throw new Error("HeaderByNumber: no implementation provided; set the mock's headerByNumber property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as HeaderByNumberReply) : fromJson(HeaderByNumberReplySchema, raw as HeaderByNumberReplyJson);
        return { response: { case: "payload", value: anyPack(HeaderByNumberReplySchema, output) } };
      }
      case "WriteReport": {
        const input = anyUnpack(req.payload, WriteReportRequestSchema) as WriteReportRequest;
        const handler = self.writeReport;
        if (typeof handler !== "function") throw new Error("WriteReport: no implementation provided; set the mock's writeReport property to define the return value.");
        const raw = handler(input);
        const output = raw && typeof (raw as unknown as { $typeName?: string }).$typeName === "string" ? (raw as WriteReportReply) : fromJson(WriteReportReplySchema, raw as WriteReportReplyJson);
