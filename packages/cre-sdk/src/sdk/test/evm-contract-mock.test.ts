...
import { TxStatus, type GasConfig } from "@cre/generated/capabilities/blockchain/evm/v1alpha/client_pb"
import { create } from "@bufbuild/protobuf"
import { type ReportResponse, ReportResponseSchema } from "@cre/generated/sdk/v1alpha/sdk_pb"
...
function makeCallContractRequest(to: Address, data: `0x${string}`) {
  return {
    $typeName:
      "capabilities.blockchain.evm.v1alpha.CallContractRequest" as const,
    call: {
...
describe("addContractMock", () => {
  describe("callContract routing", () => {
    ...
    test("chains multiple contract mocks on the same EvmMock", () => {
      ...
      evmMock.callContract!(
        makeCallContractRequest(MOCK_ADDRESS_A, callDataA) as any
      )
      ...
      evmMock.callContract!(
        makeCallContractRequest(MOCK_ADDRESS_B, callDataB) as any
      )
    ...
    test("passes decoded arguments to handler", () => {
      ...
      evmMock.callContract!(
        makeCallContractRequest(MOCK_ADDRESS_A, callData) as any
      )

      expect(receivedAccount).toBeDefined()
      expect((receivedAccount as string).toLowerCase()).toBe(
    ...
    test("throws when no mock is registered for address", () => {
      ...
      expect(() =>
        evmMock.callContract!(
          makeCallContractRequest(unknownAddress, callData) as any
        )
  ...
  describe("writeReport routing", () => {
    test("routes writeReport by receiver address", () => {
      ...
      const req = {
        $typeName:
          "capabilities.blockchain.evm.v1alpha.WriteReportRequest" as const,
        receiver: hexToUint8Array(MOCK_ADDRESS_A),
        report: { rawReport: new Uint8Array(200) },
        gasConfig: { gasLimit: 1000000n },
      }
    ...
    test("chains writeReport for multiple contracts", () => {
      ...
      mockA.writeReport = () => {
        calledA = true
        return { txStatus: TxStatus.SUCCESS }
      }
      ...
      const fakeReport = create(ReportResponseSchema, {})
      const fakeGasConfig = { gasLimit: "1000000" } as GasConfig
  ...
  describe("edge cases", () => {
    test("handles call data with exactly 4 bytes (selector only)", () => {
      ...
      const result = evmMock.callContract!(
        makeCallContractRequest(MOCK_ADDRESS_A, callData) as any
      )
      expect(result).toBeDefined()
    })
