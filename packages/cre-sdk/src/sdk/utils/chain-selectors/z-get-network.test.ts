...
describe('getNetwork', () => {
	...
	beforeAll(async () => {
		...
		// Include allNetworks for completeness even though getNetwork doesn't use it
		const allNetworks = [evmMain, evmTest, solMain, solTest]
	...
	afterAll(() => {
		// CRITICAL: Restore the original module to prevent pollution
		// Bun's mock.module persists across test files, so we must explicitly restore
