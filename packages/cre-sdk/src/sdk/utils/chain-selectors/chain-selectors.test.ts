...
describe('generated chain selectors - testnets', () => {
	it('includes sepolia by bigint selector in testnet map', async () => {
		// Use dynamic import to avoid top-level import that could be affected by mocks
		const { testnetBySelector } = await import('@cre/generated/networks')
	...
	it('includes sepolia by name in testnet map', async () => {
		// Use dynamic import to avoid top-level import that could be affected by mocks
		const { testnetByName } = await import('@cre/generated/networks')
	...
	it('allNetworks contains the sepolia network with bigint selector', async () => {
		// Use dynamic import to avoid top-level import that could be affected by mocks
		const { allNetworks } = await import('@cre/generated/networks')
